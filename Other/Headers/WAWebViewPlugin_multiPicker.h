//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "WAWebViewPluginBase.h"

#import "NSMenuDelegate-Protocol.h"

@class NSMenu, NSString;
@protocol IWAWebViewPluginDelegate;

@interface WAWebViewPlugin_multiPicker : WAWebViewPluginBase <NSMenuDelegate>
{
    id <IWAWebViewPluginDelegate> _resultDelegate;
    NSMenu *_mainMenu;
}

@property(retain, nonatomic) NSMenu *mainMenu; // @synthesize mainMenu=_mainMenu;
@property(nonatomic) __weak id <IWAWebViewPluginDelegate> resultDelegate; // @synthesize resultDelegate=_resultDelegate;
- (void).cxx_destruct;
- (void)onSelect:(long long)arg1 row:(long long)arg2;
- (void)onConfirm:(id)arg1;
- (void)onCancel;
- (id)createMenuWithLayer:(unsigned long long)arg1 itemTitles:(id)arg2;
- (void)menu:(id)arg1 willHighlightItem:(id)arg2;
- (void)onMenuItemSelected:(id)arg1;
- (void)updateMultiPickerView:(id)arg1 current:(long long)arg2 column:(long long)arg3;
- (void)showMultiPickerView:(id)arg1 current:(id)arg2;
- (BOOL)handleEvent:(unsigned long long)arg1 userInfo:(id)arg2;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

